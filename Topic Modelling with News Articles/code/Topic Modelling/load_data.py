...
#spark = SparkSession.builder.appName('Topic Modelling - Cleaning').getOrCreate()
#spark = SparkSession.builder.appName('Topic Modelling - Cleaning - NYT').getOrCreate()
    spark = SparkSession \
        .builder \
        .appName('Topic Modelling - Cleaning - NYT') \
        .config("spark.mongodb.input.uri", "mongodb://127.0.0.1/socc_db.socc_coll") \
        .config("spark.mongodb.output.uri", "mongodb://127.0.0.1/socc_db.socc_coll") \
        .getOrCreate()
    ...
    #socc_df = spark.read.format("com.mongodb.spark.sql.DefaultSource").load()
